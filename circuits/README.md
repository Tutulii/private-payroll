...
bb prove --scheme ultra_honk --oracle_hash keccak --slow_low_memory \
...
This Barretenberg release generates zero-knowledge UltraHonk proofs by default; `--disable_zk` is never used. `--slow_low_memory` reduces peak native-prover memory without changing the proof system or verifier key.
