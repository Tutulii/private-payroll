...
Status: in progress. The Mainnet topology and hosted release are deployed and
verified; one product-origin claim/remediation run remains.
...
- Linux CI enforces PostgreSQL concurrency, rendered Chromium controls, fresh
  v5/v6/v7 proof generation, every claim/remediation vector through the real
  Cairo verifiers, and the existing clean-build gates. Exact release commit
  `66488bb5ec3191022f8b327ecc8f75fff21f0534` passed PAYO CI in
  [run 33248701214](https://github.com/Tutulii/private-payroll/actions/runs/33248701214)
  and the full proof-artifacts workflow in
  [run 33248701162](https://github.com/Tutulii/private-payroll/actions/runs/33248701162).
...
- The exact release image was rolled to Fly as prover release 10 and web
  release 44. Both health checks pass, the hosted payroll and Activity routes
  serve the vNext seal, the protected prover enforces authentication/CORS, and
  the chain indexer is caught up. See
  `evidence/phase3-wage-claim-hosted-rollout.json`.
- P3-06 remains `partial` until one browser-origin Mainnet Claim v6 plus linked
  Remediation v7 reaches authorization and private-payment confirmation. It
  may be marked reconciled only if SettlementMatch or worker-authorized Ready
  note/balance evidence is recorded.
