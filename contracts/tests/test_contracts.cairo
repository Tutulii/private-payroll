...
fn cairo_commitments_match_typescript_and_noir_golden_vector() {
    ...
    let recipient = hash_recipient_commitment(0x123, 0x2222222222222222222222222222222222222222222222222222222222222222);
    let deductions = hash_deductions_commitment(array![100000_u128].span());
    let policy = hash_text_commitment(@policy_domain, @policy_id);

    assert(agreement == 0xd6bc9793865951208b221948860299f08890e5c6feabbadb2ae9f032765185d8, 'agreement vector');
    assert(recipient == 0x048f6be0cd5ce871550de4d16d62e348f5b7921c5d91f88fc9208f9511561657, 'recipient vector');
    assert(deductions == 0x9291605e0e11b42a504afc2584de5e8441dc8e3cbc53b764f91de8158a57bbc6, 'deduction vector');
    assert(policy == 0x832ef507641982f4aeb40961659420fa2d322541aae98b3a1c10a8d27af101f5, 'policy vector');
    ...
    assert(leaf == 0xe394b47d5c724e6465a5e1a522d59c20f23aa7873dad33b2cac8b50272212601, 'leaf vector');
    assert(
        build_fixed_merkle_root_v1(array![leaf].span())
...
fn pool_can_seal_a_valid_proof_and_nullifier() {
    ...
    let deposits = dispatcher.privacy_invoke(
