...
describe("official-source reference policy releases", () => {
  it("keeps every pack bounded, versioned, source-linked, and review-gated", () => {
    for (const release of REFERENCE_POLICY_RELEASES) {
      ...
      expect(release.pack.sourceUri).toMatch(/^https:\/\/(www\.)?(irs\.gov|gov\.uk|canada\.ca)/);
  ...
  it("evaluates the narrow Canadian small irregular-payment example", () => {
    expect(evaluatePolicyPack(CA_2026_SMALL_IRREGULAR_PAYMENT.pack, { gross: "500000" }))
      .toEqual({ statutoryWithholding: "75000" });
    expect(compilePolicyPack(CA_2026_SMALL_IRREGULAR_PAYMENT.pack).instructionCount).toBe(2);
    expect(policyPackCommitment(CA_2026_SMALL_IRREGULAR_PAYMENT.pack))
      .toBe("0xf8b8f3600c334d7cc22491ef61549096d83d81ef60a502dbe5a74bc67f71abd9");
  });
});
