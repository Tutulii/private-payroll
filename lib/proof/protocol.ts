...
export function mapPayrollPublicInputs(values: readonly string[]): PayrollIntegrityPublicInputs {
  ...
  // bb.js returns Noir fields as fixed-width hex strings. PAYO stores one
  // canonical representation so a proof generated by either the browser or a
  // rolling self-hosted prover can be reopened without changing its value.
  const canonical = values.map((value, index) => index < 2
    ? `0x${BigInt(value).toString(16)}`
    : BigInt(value).toString());
  return {
    chainId: canonical[0],
    sealAddress: canonical[1],
    proofVersion: canonical[2],
    schemaVersion: canonical[3],
    agreementRootHigh: canonical[4],
    agreementRootLow: canonical[5],
    manifestRootHigh: canonical[6],
    manifestRootLow: canonical[7],
    policyRootHigh: canonical[8],
    policyRootLow: canonical[9],
    fxRootHigh: canonical[10],
    fxRootLow: canonical[11],
    runNullifierHigh: canonical[12],
    runNullifierLow: canonical[13],
    validityStart: canonical[14],
    validityExpiry: canonical[15],
    shardIndex: canonical[16],
  };
}
